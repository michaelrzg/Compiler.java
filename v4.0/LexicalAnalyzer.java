...
public class LexicalAnalyzer {
    ...
    public Token getToken() throws InvalidTokenException {
        ...
        if (pointerX >= src.length()) {
            return new Token("EOS", Types.EOS, pointerX++, pointerY++);
        }
        ...
        switch (src.charAt(pointerX)) {
            ...
            case '=' -> {
                return new Token("" + src.charAt(pointerX), Types.ASSIGNMENT, pointerX++, pointerY);
            }
            default -> {
                // if its not one of the symbols above AND it is an ascii integer literal or id
                ...
                if (src.charAt(pointerX) > 47 && src.charAt(pointerX) < 58) {
                ...
                } // case that this is an ID
                else if (src.charAt(pointerX)>64 && src.charAt(pointerX)<123){
                    String id = "";
                    int start = pointerX;
                    while(src.charAt(pointerX)>64 && src.charAt(pointerX)<123){
                        id+=src.charAt(pointerX);
                        pointerX++;
                        if(pointerX>= src.length()){
                            break;
                        }
                    ...
                    if(id.equals("display")){
                        return new Token(id,Types.DISPLAY,start,pointerY);
                    }
                    else if(id.equals("input")){
                        return new Token(id,Types.INPUT,start,pointerY);
                    }
                    return new Token(id,Types.ID,start,pointerY);
